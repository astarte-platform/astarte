...
"""
  ...
  def install_realm_interfaces() do
    IO.inspect File.cwd!
    cmd = "cd " <> File.cwd! <> "/test/support/ && ./install_realm.sh" 
    cmd = String.to_atom(cmd)
    output = :os.cmd(cmd)
    IO.puts("The output of realm installtion is #{output}")
    :timer.sleep(5000)
    cmd = "cd " <> File.cwd! <> "/test/support/ && ./install_interfaces.sh"
    cmd = String.to_atom(cmd)
    output = :os.cmd(cmd)
    IO.puts("The output of interface installtion is #{output}")
  end 
end
