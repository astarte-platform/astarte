...
@drop_keyspace  """
 ...
 def initialize_database() do
   ...
   IO.puts("Database connection established")

   Xandra.execute(conn, @drop_keyspace, [], []) 
   IO.puts("dropped the keyspace")
   :timer.sleep(3000)
   ...
   {:ok,
       %Xandra.SchemaChange{
          ...
          target: "KEYSPACE",
          tracing_id: nil}} = Xandra.execute(conn, @create_test, [], [])

   :timer.sleep(2000)
  
   create_tables = [@create_kv_store,
                    @create_names_table,
                    @create_groups_table,
                    @create_devices_table,
                    @create_interfaces_table,
