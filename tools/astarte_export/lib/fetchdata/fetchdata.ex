defmodule Astarte.Export.FetchData do
  ...
  def fetch_device_data(conn, realm, opts) do
    with {:ok, result} <- Queries.stream_devices(conn, realm, opts),
      [device_data | _]  = result_list <- Enum.to_list(result) do
  ...
  def process_device_data(device_data) do
    ...
    first_registration =
      DateTime.to_iso8601(device_data.first_registration)
    ...
    first_credentials_request =
      DateTime.to_iso8601(device_data.first_credentials_request)
  ...
  def fetch_object_datastreams(conn, realm, interface_info, options) do
    ...
    with {:ok, result} <- 
           Queries.retrieve_object_datastream_value(conn, realm, storage, device_id, path, options) do
      updated_options = Keyword.put(options, :paging_state, result.paging_state)
