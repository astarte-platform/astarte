pub type AstarteClient = DeviceClient<Mqtt<SqliteStore, PairingApi>>;
...
pub async fn check_add(api: &ApiClient, client: &mut AstarteClient) -> eyre::Result<()> {
...
pub async fn check_remove(api: &ApiClient, client: &mut AstarteClient) -> eyre::Result<()> {
...
pub async fn retry<F, T, U>(times: usize, mut f: F) -> eyre::Result<U>
where
    F: FnMut() -> T,
