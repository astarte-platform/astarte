  ...
  def run(args) do
    ...
    auth_token =
      case Keyword.has_key?(opts, :auth_token) do
        ...
        false ->
          with private_key_path <- opts[:realm_private_key],
               {:ok, abs_path} <- Path.path_from(private_key_path),
    ...
    with dashboard_url <- opts[:dashboard_url],
         realm_name <- opts[:realm_name],
         {:ok, authenticated_url} = Dashboard.Open.exec(realm_name, dashboard_url, auth_token) do
