...
defmodule AstarteE2E.Device do
  ...
  def init(opts) do
    ...
    params = opts |> Keyword.delete(:device_id) |> Keyword.put(:id, device_id)
    device = DeviceGenerator.device(params)
    ...
    interface_provider_introspection = interfaces |> Map.new(&{&1["interface_name"], &1})

    interface_provider =
      {Astarte.Device.SimpleInterfaceProvider, interfaces: interface_provider_introspection}
  ...
  defp register_device!(realm, device, interfaces) do
    ...
    introspection =
      ...
      |> Map.new(
        &{&1["interface_name"],
         %{
           "major" => &1["version_major"],
           "minor" => &1["version_minor"]
         }}
      )

    url = "#{astarte_pairing_url}/v1/#{realm}/agent/devices"
  ...
  defp device_name(realm, device_id), do: {:via, Registry.AstarteE2E, {:device, realm, device_id}}
end
