...
defmodule Astarte.FDO.Core.OwnerOnboarding.SessionKey do
  @moduledoc """
  Module for handling the SessionKey structure in the FDO Owner Onboarding protocol,
  ...
  def compute_shared_secret(dhkex_alg, _key, owner_random, xb)
    ...
    {dhkex_p, key_size} =
      case dhkex_alg do
        "DHKEXid14" -> {@dhkex_p_2048, 256}
        "DHKEXid15" -> {@dhkex_p_3072, 384}
      end
    ...
    # :crypto.mod_pow/3 strips leading zero bytes; pad back to the prime's full byte length
    {:ok, :binary.copy(<<0>>, key_size - byte_size(shse)) <> shse}
  end
