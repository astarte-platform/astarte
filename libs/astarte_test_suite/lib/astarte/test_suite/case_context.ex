...
defmodule Astarte.TestSuite.CaseContext do
  ...
  defp missing_keys(_context, [], acc), do: :lists.reverse(acc)

  defp missing_keys(context, [key | rest], acc) do
    case Map.has_key?(context, key) do
      true -> missing_keys(context, rest, acc)
      false -> missing_keys(context, rest, [key | acc])
    end
  end
  ...
  defp unknown_keys([], _allowed_keys, acc), do: :lists.reverse(acc)

  defp unknown_keys([key | rest], allowed_keys, acc) do
    case member?(key, allowed_keys) do
      true -> unknown_keys(rest, allowed_keys, acc)
      false -> unknown_keys(rest, allowed_keys, [key | acc])
    end
  end
