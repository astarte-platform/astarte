...
defmodule Astarte.TestSuite do
  ...
  A test module uses the suite directly:
      ...
      use Astarte.TestSuite,
        ...
        device: [devices: [%Astarte.TestSuite.Mock.Device{}]],
  ...
  `group: true` loads `common`, `database`, `interface`, `device`, and `group`
  in that order.
  ...
  alias Astarte.TestSuite.Cases.Database, as: DatabaseCase
  ...
  defp quoted_uses([], acc), do: :lists.reverse(acc)
  ...
  defp expanded_external_modules([], _caller, acc), do: :lists.reverse(acc)
  ...
  defp parse_runtime_external_cases([], acc), do: :lists.reverse(acc)
  ...
  defp parse_ast_external_cases([], _caller, acc), do: :lists.reverse(acc)
  ...
  defp graph_cases([], _included_cases, _requests, acc), do: :lists.reverse(acc)
  ...
  defp load_order([], acc), do: :lists.reverse(acc)
  ...
  defp take_until_including([], _value, acc), do: :lists.reverse(acc)

  defp take_until_including([value | _rest], value, acc),
    do: :lists.reverse([value | acc])

  defp take_until_including([head | rest], value, acc),
    do: take_until_including(rest, value, [head | acc])

  defp format_path(path), do: join_inspected(path, "")

  defp join_inspected([], acc), do: acc
  defp join_inspected([value], ""), do: inspect(value)
  defp join_inspected([value], acc), do: acc <> " -> " <> inspect(value)
  defp join_inspected([value | rest], ""), do: join_inspected(rest, inspect(value))
  ...
  defp keyword_ast?(_list), do: false
