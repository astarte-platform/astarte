...
defmodule Astarte.FDO.Config do
  ...
  alternative to `base_url_domain`. At least one between this and `base_url_domain` must be
  configured.
  """
  ...
  def base_url! do
    ...
    host = __MODULE__.base_url_host!() |> url_host()
  ...
  # An IPv6 address must be bracketed to be used in a URL
  defp url_host(host) do
    case (host || "") |> String.to_charlist() |> :inet.parse_ipv6strict_address() do
      {:ok, _address} -> "[#{host}]"
      {:error, _reason} -> host
    end
  end
