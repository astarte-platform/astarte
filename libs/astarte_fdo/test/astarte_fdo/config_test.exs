...
defmodule Astarte.FDO.ConfigTest do
  ...
  describe "base_url!/0" do
    ...
    test "brackets an IPv6 address so that the URL stays parseable" do
      stub(Config, :base_url_protocol!, fn -> :http end)
      stub(Config, :base_url_domain!, fn -> nil end)
      stub(Config, :base_url_ip!, fn -> "2001:db8::1" end)
      stub(Config, :base_url_port!, fn -> 4003 end)

      url = Config.base_url!()

      assert url == "http://[2001:db8::1]:4003"
      assert {:ok, %URI{host: "2001:db8::1", port: 4003}} = URI.new(url)
    end

    test "does not bracket a domain that is not an IPv6 address" do
      stub(Config, :base_url_protocol!, fn -> :https end)
      stub(Config, :base_url_domain!, fn -> "astarte.example.com" end)
      stub(Config, :base_url_ip!, fn -> nil end)
      stub(Config, :base_url_port!, fn -> 443 end)

      assert {:ok, %URI{host: "astarte.example.com", port: 443}} =
               URI.new(Config.base_url!())
    end
  end
