...
defmodule Astarte.FDO.Helpers do
  @moduledoc """
  ...
  defp get_curve_params(:p256) do
    ...
    {{1, 2, 840, 10_045, 3, 1, 7}, :sha256, {:sha256, :ecdsa}, -7, :secp256r1}
  end
  ...
  defp generate_self_signed_cert(pub_key_point, priv_key, oid, sig_alg) do
    sig_oid =
      case sig_alg do
        {:sha256, :ecdsa} -> {1, 2, 840, 10_045, 4, 3, 2}
        {:sha384, :ecdsa} -> {1, 2, 840, 10_045, 4, 3, 3}
      end
    ...
    spki =
      {:OTPSubjectPublicKeyInfo,
       {:PublicKeyAlgorithm, {1, 2, 840, 10_045, 2, 1}, {:namedCurve, oid}},
