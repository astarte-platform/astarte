defmodule Astarte.AppEngine.API.DeviceTest do
  ...
  describe "devices_by_alias" do
    ...
    def device_status_by_alias_fixture(attrs \\ %{}) do
      {:ok, device_status_by_alias} =
        attrs
        |> Enum.into(@valid_attrs)
        |> AppEngine.API.Device.create_device_status_by_alias()

      device_status_by_alias
    end

    test "list_devices_by_alias/0 returns all devices_by_alias" do
      device_status_by_alias = device_status_by_alias_fixture()
      assert AppEngine.API.Device.list_devices_by_alias() == [device_status_by_alias]
    end

    test "get_device_status_by_alias!/1 returns the device_status_by_alias with given id" do
      device_status_by_alias = device_status_by_alias_fixture()
      assert AppEngine.API.Device.get_device_status_by_alias!(device_status_by_alias.id) == device_status_by_alias
    end

    test "create_device_status_by_alias/1 with valid data creates a device_status_by_alias" do
      assert {:ok, %DeviceStatusByAlias{} = device_status_by_alias} = AppEngine.API.Device.create_device_status_by_alias(@valid_attrs)
    end

    test "create_device_status_by_alias/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = AppEngine.API.Device.create_device_status_by_alias(@invalid_attrs)
    end

    test "update_device_status_by_alias/2 with valid data updates the device_status_by_alias" do
      device_status_by_alias = device_status_by_alias_fixture()
      assert {:ok, device_status_by_alias} = AppEngine.API.Device.update_device_status_by_alias(device_status_by_alias, @update_attrs)
      assert %DeviceStatusByAlias{} = device_status_by_alias
    end

    test "update_device_status_by_alias/2 with invalid data returns error changeset" do
      device_status_by_alias = device_status_by_alias_fixture()
      assert {:error, %Ecto.Changeset{}} = AppEngine.API.Device.update_device_status_by_alias(device_status_by_alias, @invalid_attrs)
      assert device_status_by_alias == AppEngine.API.Device.get_device_status_by_alias!(device_status_by_alias.id)
    end

    test "delete_device_status_by_alias/1 deletes the device_status_by_alias" do
      device_status_by_alias = device_status_by_alias_fixture()
      assert {:ok, %DeviceStatusByAlias{}} = AppEngine.API.Device.delete_device_status_by_alias(device_status_by_alias)
      assert_raise Ecto.NoResultsError, fn -> AppEngine.API.Device.get_device_status_by_alias!(device_status_by_alias.id) end
    end

    test "change_device_status_by_alias/1 returns a device_status_by_alias changeset" do
      device_status_by_alias = device_status_by_alias_fixture()
      assert %Ecto.Changeset{} = AppEngine.API.Device.change_device_status_by_alias(device_status_by_alias)
    end
  end
end
