defmodule Astarte.AppEngine.APIWeb.InterfacePlug do
  ...
  Everything will be handled by phoenix router later.
  """
  def call(%Plug.Conn{path_info: ["v1", _realm_name, "devices-by-alias", _device_alias, "interfaces", _interface, _path_component_1 | _]} = conn, _opts) do
    ["v1", realm_name, "devices-by-alias", device_alias, "interfaces", interface | subpath] = conn.path_info
    original_path_info = conn.path_info
    ...
    %{conn |
      ...
      query_params: new_query_params,
      query_string: new_query_string,
      request_path: "/v1/#{realm_name}/devices-by-alias/#{device_alias}/interfaces/#{interface}?#{new_query_string}"
    }
    |> Conn.assign(:original_path_info, original_path_info)
  end
