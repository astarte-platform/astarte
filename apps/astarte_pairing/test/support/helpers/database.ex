...
defmodule Astarte.Support.Helpers.Database do
  @moduledoc """
  The module contains all the queries needed to create the database for the Astarte system.
  It is used as a `helper` for the tests
  """
  ...
  def setup_astarte!(astarte_keyspace) do
  ...
  def teardown_astarte!(astarte_keyspace) do
    execute!(astarte_keyspace, @drop_keyspace)
    :ok
  end

  def setup_realm!(astarte_keyspace, realm_name) do
  ...
  def teardown_realm!(astarte_keyspace, realm_name) do
    realm_keyspace = Realm.keyspace_name(realm_name)

    %Realm{realm_name: realm_name}
    |> Repo.delete!(prefix: astarte_keyspace)
  ...
  #
  # Common function
  defp execute!(keyspace, query) do
