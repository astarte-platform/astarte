...
defmodule Astarte.Pairing.FDOClient do
  ...
  def to0_hello() do
    ...
    Logger.info("Sending TO0.Hello to FDO rendezvous server...", url: url)

    case http_client().post(url, request_body, headers) do
      {:ok, %{status_code: 200, headers: headers, body: body}} ->
        Logger.info("TO0.Hello completed successfully")
        Logger.info(inspect(headers))

        case TO0Util.get_nonce_from_hello_ack(body) do
          {:ok, nonce} ->
            Logger.info("Nonce retrieved from TO0.HelloAck", nonce: Base.encode16(nonce))
  ...
  defp get_auth_bearer(headers) do
    Enum.find_value(headers, fn
      {"authorization", value} ->
        Logger.info("Authorization Bearer retrieved: #{inspect(value)}")
        value
  ...
  def to0_owner_sign(nonce, bearer) do
    Logger.info("Sending TO0.OwnerSign...")
    ...
    with {:ok, owner_key} <- TO0Util.get_owner_private_key(),
      ...
      Logger.info("Sending TO0.OwnerSign", payload_size: byte_size(request_body))

      case http_client().post(url, request_body, headers) do
        {:ok, %{status_code: 200, body: _body}} ->
          Logger.info("TO0.OwnerSign completed successfully!")
