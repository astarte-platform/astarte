...
defmodule Astarte.RealmManagement.Engine do
  ...
  defp verify_mappings_max_storage_retention(realm_name, interface) do
    with {:ok, max_retention} <- get_datastream_maximum_storage_retention(realm_name),
         {:mappings_retention_valid, true} <-
           {:mappings_retention_valid,
            mappings_retention_valid?(interface.mappings, max_retention)} do
      :ok
    else
      {:mappings_retention_valid, false} -> {:error, :maximum_database_retention_exceeded}
    end
  end
