...
defmodule Astarte.RealmManagement.Queries do
  ...
  def is_any_device_using_interface?(realm_name, interface_name) do
    ...
    with {:ok, _result} <- Repo.fetch_one(devices_query, prefix: keyspace, consistency: :quorum) do
      {:ok, true}
    else
      {:error, :not_found} ->
        {:ok, false}
  ...
  def devices_with_data_on_interface(realm_name, interface_name) do
    ...
    {:ok, Repo.all(query, prefix: keyspace, consistency: :quorum)}
  end
  ...
  defp fetch_all_paths_and_endpoint_ids(
    ...
    properties =
      query
      |> Repo.all(prefix: keyspace, consistency: :quorum)
      |> Enum.map(fn property -> [endpoint_id: property.endpoint_id, path: property.path] end)
  ...
  def is_interface_major_available?(realm_name, interface_name, interface_major) do
    ...
    case Repo.all(query, prefix: keyspace, consistency: :quorum) do
      [] -> {:ok, false}
      _non_empty -> {:ok, true}
    end
  end
  ...
  def check_interface_name_collision(realm_name, interface_name) do
    ...
    names = Repo.all(all_names_query, prefix: keyspace, consistency: :quorum)
  ...
  def fetch_interface(realm_name, interface_name, interface_major) do
    ...
    with {:ok, interface} <-
      ...
      endpoints_query =
        from Endpoint,
          where: [interface_id: ^interface.interface_id]

      mappings =
        Repo.all(endpoints_query,
          prefix: keyspace,
          consistency: :quorum
        )
  ...
  def get_interfaces_list(realm_name) do
    ...
    interfaces = Repo.all(query, prefix: keyspace, consistency: :quorum)
  ...
  def has_interface_simple_triggers?(realm_name, object_id) do
    ...
    exists? =
      case Repo.all(simple_triggers_query, prefix: keyspace) do
        [] -> false
        _non_empty -> true
      end
  ...
  def get_jwt_public_key_pem(realm_name) do
    ...
    query =
      from store in KvStore,
        select: store.value,
        where: [group: "auth", key: "jwt_public_key_pem"]

    try do
      Repo.fetch_one(query, consistency: :quorum, prefix: keyspace)
    rescue
      error in Xandra.Error -> handle_xandra_error(error)
    end
  end
  ...
  def retrieve_trigger_uuid(realm_name, trigger_name, format \\ :string) do
    ...
    with {:ok, uuid} <- Repo.fetch_one(query, prefix: keyspace, error: :trigger_not_found) do
      case format do
        :string ->
