...
defmodule Astarte.Housekeeping.Migrator do
  ...
  defp ensure_astarte_kv_store do
    ...
    WHERE keyspace_name='#{Config.astarte_instance_id!()}_astarte' AND table_name='kv_store'
    """
  ...
  defp create_astarte_kv_store do
    query = """
    CREATE TABLE #{Config.astarte_instance_id!()}_astarte.kv_store (
      group varchar,
      key varchar,
      value blob,
  ...
  defp get_astarte_schema_version do
    Xandra.Cluster.run(:xandra, fn conn ->
      with :ok <- use_keyspace(conn, "#{Config.astarte_instance_id!()}_astarte") do
  ...
  defp migrate_astarte_keyspace_from_version(current_schema_version) do
    ...
    Xandra.Cluster.run(:xandra, [timeout: :infinity], fn conn ->
      with :ok <- use_keyspace(conn, "#{Config.astarte_instance_id!()}_astarte"),
