...
defmodule Astarte.Core.Generators.Triggers.TriggerTest do
  ...
  describe "trigger generator" do
    property "generates valid triggers" do
      check all %Trigger{
        ...
        assert trigger == trigger |> Trigger.encode() |> Trigger.decode()
        assert uuid_v4?(trigger_uuid)
        assert length(simple_triggers_uuids) == 1
        assert Enum.all?(simple_triggers_uuids, &uuid_v4?/1)
        assert trigger_uuid not in simple_triggers_uuids
        assert Map.fetch!(action, "http_method") in ~w(delete get head options patch post put)
        assert scheme in ["http", "https"]
        assert byte_size(name) in 1..128
        assert is_nil(policy)
      end
    end

    test "uses bottom-up identifiers and policy names" do
      assert %Trigger{
               version: 0,
               trigger_uuid: @trigger_uuid,
