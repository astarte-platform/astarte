...
defmodule Astarte.Core.Generators.Triggers.Trigger do
  @moduledoc """
  Generates Astarte triggers with valid HTTP actions.
  """
  ...
  `simple_triggers_uuids` and `policy` parameters.
  """
  ...
  def trigger(params \\ []) do
    params gen all version <- constant(0),
                   trigger_uuid <- uuid(),
                   simple_triggers_uuids <- simple_trigger_ids(trigger_uuid),
                   action <- action(),
                   name <- name(),
                   policy <- constant(nil),
                   params: params do
  ...
  defp simple_trigger_ids(trigger_uuid),
    do:
      uuid()
      |> filter(&(&1 != trigger_uuid))
      |> map(&[&1])
