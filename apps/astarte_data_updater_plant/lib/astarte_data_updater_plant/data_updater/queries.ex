...
defmodule Astarte.DataUpdaterPlant.DataUpdater.Queries do
  ...
  @purge_properties_key "purge_properties_compression_format"
  ...
  defp do_fetch_device_capabilities(conn, realm, device_id) do
    ...
    with {:ok, prepared} <- Xandra.prepare(conn, statement),
      ...
      {:error, %Xandra.Error{} = error} ->
        _ =
          Logger.warning(
            "Database error while fetching device #{inspect(device_id)} capabilities: #{Exception.message(error)}"
          )
      ...
      {:error, %Xandra.ConnectionError{} = error} ->
        _ =
          Logger.warning(
            "Database connection error while fetching device #{inspect(device_id)} capabilities: #{Exception.message(error)}"
          )
  ...
  defp map_to_capabilities(change) do
    change =
      case Map.get(change, @purge_properties_key) do
        nil -> Map.put(change, @purge_properties_key, :zlib)
        other -> change
      end
