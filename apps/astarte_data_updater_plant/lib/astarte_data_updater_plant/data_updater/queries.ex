...
defmodule Astarte.DataUpdaterPlant.DataUpdater.Queries do
  ...
  def set_pending_empty_cache(realm, device_id, pending_empty_cache) do
    ...
    case Repo.safe_update(device, []) do
      {1, _} ->
        :ok
  ...
  defp set_connection_info!(realm, device_id, timestamp, ip_address) do
    ...
    changeset = compute_device_update_changes(device_id, changes)

    Repo.update!(changeset, prefix: keyspace_name, consistency: :local_quorum)
  end

  defp refresh_device_connected!(realm, device_id, ttl) do
    keyspace_name = Realm.keyspace_name(realm)

    changeset = compute_device_update_changes(device_id, %{connected: true})
  ...
  def update_device_introspection!(realm, device_id, introspection, introspection_minor) do
    ...
    Repo.update!(changeset, prefix: keyspace_name, consistency: :quorum)
  end
  ...
  def add_old_interfaces(realm, device_id, old_interfaces) do
    ...
    case Repo.safe_update(device, [], consistency: :quorum) do
      {1, _} ->
        :ok
  ...
  defp compute_device_update_changes(device_id, changes) do
    Ecto.Changeset.cast(%Device{device_id: device_id}, changes, Map.keys(changes))
  end
