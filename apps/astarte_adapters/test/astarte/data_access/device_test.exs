...
defmodule Astarte.DataAccess.Adapters.DeviceTest do
    ...
    property "maps a data access device" do
      check all %{
        ...
        for field <- @direct_fields do
          assert Map.get(changes, field) == Map.get(device, field)
        end
        ...
        assert %{groups: %{}} =
                 from_core_device_to_change(%{device: device, interfaces: interfaces})
      end
    end
  end
  ...
  defp adapted_device do
    gen all interfaces <- interface() |> list_of(max_length: 5),
      ...
      %{device_id: device_id} = device = Map.merge(device, optional_fields())
