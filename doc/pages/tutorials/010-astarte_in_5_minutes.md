...
```sh
$ git clone https://github.com/astarte-platform/astarte.git -b v1.0.1 && cd astarte
$ docker run -v $(pwd)/compose:/compose astarte/docker-compose-initializer:1.0.1
$ docker-compose up -d
```
...
```sh
$ docker run --net="host" -e "DEVICE_ID=$(astartectl utils device-id generate-random)" -e "PAIRING_URL=http://localhost:4003" -e "REALM=test" -e "PAIRING_JWT=$(astartectl utils gen-jwt pairing -k test_private.pem)" -e "IGNORE_SSL_ERRORS=true" astarte/astarte-stream-qt5-test:1.0.1
```
...
Refer to `stream-qt5-test` [README](https://github.com/astarte-platform/stream-qt5-test/blob/v1.0.1/README.md) for more details on which variables can be passed to the container.
