...
```sh
$ docker pull astarte/astarte-stream-qt5-test:1.0.0
```

Its most basic invocation (from your `astarte` repository tree) is:

```sh
$ docker run --net="host" -e "DEVICE_ID=$(astartectl utils device-id generate-random)" -e "PAIRING_HOST=http://localhost:4003" -e "REALM=test" -e "AGENT_KEY=$(astartectl utils gen-jwt pairing -k test_private.pem)" -e "IGNORE_SSL_ERRORS=true" astarte/astarte-stream-qt5-test:1.0.0
```
...
Refer to `stream-qt5-test` [README](https://github.com/astarte-platform/stream-qt5-test/blob/v1.0.0/README.md) for more details on which variables can be passed to the container.
