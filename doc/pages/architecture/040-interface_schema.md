...
|**database_retention_policy**|`string`|Useful only with datastream. Defines whether data is expired from the database after a given time to live interval.|No, default: `"no_ttl"`|
...
Useful only with datastream. Defines whether data is expired from the database after a given time to live interval. When `"no_ttl"` is used data are not expired.
